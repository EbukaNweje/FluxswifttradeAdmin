...
const ManageUsers = () => {
  ...
  const getAllUserData = () => {
    const url =
      "https://fluxswifttrade-back-end-two.vercel.app/api/alluserdata";
    axios
      .get(url)
      .then((response) => {
  ...
  //     email: email.value,
  //   };
  //   fetch('https://fluxswifttrade-back-end-two.vercel.app/api/signupemailsand', {
  //     method: 'POST',
  //     headers: {
  ...
  const adminAproveEmailSand = async (email) => {
    ...
    fetch(
      "https://fluxswifttrade-back-end-two.vercel.app/api/adminAproveEmailSand",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  const Verify = (id) => {
    const url = `https://fluxswifttrade-back-end-two.vercel.app/api/UserVerify/${id}`;
    axios
      .patch(url)
      .then((response) => {
  ...
  return (
    <>
      <div className="w-full h-max px-6 py-10 flex flex-col gap-2 phone:gap-8 bg-[#f9fbfd] text-[rgb(87,89,98)]">
        <p className="text-[27px] font-semibold">Fluxswifttrade users list</p>
